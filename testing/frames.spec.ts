...
test("Handling frames", async function({page}){
    ...
    // Go forward to the next page - goForward()
    // await page.goForward() // move to the next page

    await page.getByText("Iframe with in an Iframe", {exact:true}).click()

    // clear() - Clear the input field i.e., Deletes any text currently typed inside that box so it's completely empty
...
// Upload the files - Wednesday
// Download file - Wednesday
// Calendar selection - Thursday
// codegen - Thursday
// E2E - End to End scenario - Next week session
